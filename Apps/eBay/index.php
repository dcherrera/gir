...
//		if(!$mancount['ModelNumber']){
//			echo ('<td colspan="3">Sorry you can\'t search without a model number</td>');
//		}else{
	  		echo('<td colspan="3"><a href="'.$dir.'/ebayValue.php?modelNum='.$mancount['ModelNumber'].'&id='.$row['ItemID'].'&man='.$mancount['Manufacturer'].'" target="_blank">Click to Get Value Then click eBay on left</a></td>');
//		}
	} else {
		if($row['EstimatedValue'] == 0.00){
		echo ('<td colspan="3">Sorry no viable sales. Please double check ebay and report bugs.');
		echo('<a href="'.$dir.'/ebaysearch.php?modelNum='.$mancount['ModelNumber'].'&man='.$mancount['Manufacturer'].'" target="_blank">See how we got this</a><br></td>');
		}else{
	  	echo('<td colspan="3">Estimated eBay Value: $'.$row['EstimatedValue'].'<br>');
	  	echo('<a href="'.$dir.'/ebaysearch.php?modelNum='.$mancount['ModelNumber'].'&man='.$mancount['Manufacturer'].'" target="_blank">See how we got this</a><br>');
	  	echo('<a href="'.$dir.'/ebayValue.php?modelNum='.$mancount['ModelNumber'].'&id='.$row['ItemID'].'&man='.$mancount['Manufacturer'].'" target="_blank">Update</a></td>');
		}
	}
	  echo '</tr>';
}
